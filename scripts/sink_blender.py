...
class Sink(Network.node):
    def __init__(self):
        ...
        self.response_cmds = {
            "wave": "bash /home/ergocub/perception/scripts/wave_hand_blender.bash",
            "shake": "bash /home/ergocub/perception/scripts/shake_blender.bash",
            "reset_wave": "bash /home/ergocub/perception/scripts/resetWave.bash",
            "reset_shake": "bash /home/ergocub/perception/scripts/resetHandshake.bash",
        }
            ...
            if self.obj_distance is Signals.NOT_OBSERVED or self.obj_distance/1000 > 1.5:  # No box in 1 meter
                if self.action != 'none':
                    if self.action in self.response_cmds and self.action != self.prev_action:
                        if time.time() - self.cooldown > self.current_action_duration:
                            cmd = self.response_cmds[f"reset_{self.action}"]
                            subprocess.run(cmd, shell=True)
                            cmd = self.response_cmds[self.action]
                            subprocess.run(cmd, shell=True)	
                            self.cooldown = time.time()
                            self.current_action_duration = self.action_duration[self.action]
